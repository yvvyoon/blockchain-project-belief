...
docker-compose -f docker-compose.yml up -d orderer.example.com ca.example.com  peer0.org1.example.com couchdb cli peer1.org1.example.com cli2 peer0.org2.example.com cli3 peer1.org2.example.com cli4
docker ps
...
#docker exec -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org1.example.com/msp" peer1.org2.example.com peer channel join -b /etc/hyperledger/configtx/mychannel.block
docker exec  peer1.org2.example.com peer channel join -b /etc/hyperledger/configtx/mychannel.block
